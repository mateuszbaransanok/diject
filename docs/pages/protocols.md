...
Protocols are used to **extend the functionality of providers** and enable execute operations on
them. They define specific behaviors that providers can implement, allowing for better control and
lifecycle management.

The following protocols define different behaviors for dependency providers:

- **`StartProtocol`** – Ensures providers can be started when needed.
- **`ResetProtocol`** – Enables resetting a provider’s internal state.
- **`ScopeProtocol`** – Manages resource cleanup at the end of a scope.
- **`StatusProtocol`** – Allows checking the current status of a provider.

Each protocol has **synchronous (`sync`) and asynchronous (`async`) counterparts**, providing
...
The `StartProtocol` ensures that providers can be **started at the appropriate time**, making sure
...
By enforcing **structured initialization**, the `StartProtocol` helps maintain **predictable,
...
The `ResetProtocol` enables resetting the **state** of a provider, allowing it to be
...
The `ScopeProtocol` ensures that resources are **properly managed and released** at the end of their
...
When the **context (`with` or `async with`) exits**, the `ScopeProtocol` ensures that any cleanup
...
By leveraging the `ScopeProtocol`, dependency injection becomes **more efficient, predictable, and
...
The `StatusProtocol` allows checking the **current status** of a provider. Providers that maintain
an internal state should implement this protocol to indicate whether they are **active, stopped, or
...
By implementing `StatusProtocol`, applications gain **better observability and control** over
...
Protocols in dependency injection systems are designed to **extend the functionality** of providers
and offer **better control and management** over their lifecycle. By implementing these protocols,
...
The **StartProtocol** ensures that providers can be **initialized** at the appropriate time,
...
The **ResetProtocol** allows providers to **reset their internal state**, enabling them to be
...
The **ScopeProtocol** provides mechanisms for managing resources **within a defined scope**,
...
Finally, the **StatusProtocol** allows you to **monitor the status** of a provider, checking if it
...
Together, these protocols form a powerful toolkit for managing dependency lifecycles, ensuring that
